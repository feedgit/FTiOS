...
public class FTKeyChainService: NSObject, FTCoreServiceComponent {
    
    func setup() {
        
    }
    
    public static let shared = FTKeyChainService()
    ...
    public func accessToken(value: String) {
        save(key: "accessToken", data: value)
    }
    ...
    public func save(key: String, data: String) {
        ...
        FTSimpleCache.shared.save(key:bundleKey, value: String(data))
    }
    ...
    public func delete(_ key: String) {
        ...
        FTSimpleCache.shared.delete(key: bundleKey)
    }
    ...
    public func get(key: String) -> String? {
        ...
        if(FTSimpleCache.shared.isExists(key:bundleKey)) {
            return FTSimpleCache.shared.get(key:bundleKey)!
        }
        ...
        if (status == errSecSuccess) {
            if let retrievedData = extractedData as? Data {
                if let value = String(data: retrievedData, encoding: String.Encoding.utf8) {
                    FTSimpleCache.shared.save(key:bundleKey, value: value)
                    return value
                }
            }
        }
