...
public extension UIImageView {
        ...
        let task = URLSession.shared.dataTask(with: url) { (data, _ , _) in
            DispatchQueue.main.async {
                ...
                if let data = data {
                    ...
                    self.gifDelegate?.gifURLDidFinish?(sender: self)
                } else {
                    self.gifDelegate?.gifURLDidFail?(sender: self)
                }
            }
        }
        task.resume()
    ...
    public var gifDelegate: SwiftyGifDelegate? {
        get {
    ...
    fileprivate var isPlaying: Bool {
        ...
        set {
            ...
            if newValue {
                self.gifDelegate?.gifDidStart?(sender: self)
            } else {
                self.gifDelegate?.gifDidStop?(sender: self)
            }
        }
    }
