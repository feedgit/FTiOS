//
//  FTAlertViewManager.swift
//  FeedTrue
//
//  Created by Quoc Le on 9/6/18.
//  Copyright © 2017 Appa Rao Mulpuri. All rights reserved.
//

import UIKit

struct FTAlertControllerInfo {
    var presentingVC : UIViewController
    var alertController : UIAlertController
}

public class FTAlertViewManager: NSObject {
    public class var defaultManager : FTAlertViewManager {
        struct Static {
            static let instance : FTAlertViewManager = FTAlertViewManager()
        }
        return Static.instance
    }
    
    fileprivate var alertCtrls = [FTAlertControllerInfo]()
    ...
    public func showLogoutAlert() {
        FTAlertViewManager.defaultManager.showOkAlert("User Logout!", message: "Someone has used your credentials to login into HealthePets. Please click on OK to login again.") { (response) in
    ...
    fileprivate func presentAlertController(_ alertController : UIAlertController, presentingViewController : UIViewController) {
        DispatchQueue.main.async { () -> Void in
            ...
            if let presentedModalVC = presentingViewController.presentedViewController {
                presentedModalVC.present(alertController, animated: true, completion: nil)
                self.alertCtrls.append(FTAlertControllerInfo(presentingVC:presentedModalVC, alertController: alertController))
            }
            else {
                presentingViewController.present(alertController, animated: true, completion: nil)
                self.alertCtrls.append(FTAlertControllerInfo(presentingVC:presentingViewController, alertController: alertController))
            }
        }
    }
